...
struct MovieCell: View {
    ...
    var body: some View {
        HStack (alignment: .bottom, spacing: 27) {
            Image(uiImage: movieData.getPosterImage())
                .resizable()
            ...
            VStack(alignment: .leading) {
                ...
                HStack {
                    Text(String(format: "%.0f", movieData.movieInfo.getVotePercentage()) + "%")
                ...
                Spacer()
                GenresView(items: movieData.movieInfo.getGenreStrings())
            }
        }
...
#Preview {
    MovieCell(movieData: MovieData(posterImage: UIImage(named: "Oppenheimer")?.pngData() ?? Data(), backDropImage: UIImage(named: "OppenheimerBackDrop")?.pngData() ?? Data(), movieInfo: MovieModel(genre_ids: [28, 35, 28, 28], release_date: "2023-08-21",title: "Oppenheimer", vote_average: 7.921)))
}
